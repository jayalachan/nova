...
# -----------------------------
    ...
    with tab3:
        st.markdown("### 📋 Application Requirements x")
...
# -----------------------------
    ...
    st.header("📝 Apply for your NovaCard x")
...
# -----------------------------
    ...
    if len(applications) == 0:
        ...
        # View options
        view_type = st.radio("Select view:", ["📋 Table", "🔍 Full Details"], horizontal=True)
        
        if view_type == "📋 Table":
            # Summary table view
            st.markdown("### Applications Summary")
            ...
            for app in applications:
                ...
                with col1:
                    st.write(f"**#{app['id']}**")
                with col2:
                    st.write(app['full_name'])
                with col3:
                    st.write(app['email'])
                with col4:
                    st.write(app['application_date'])
                st.markdown("---")
            ...
            for app in applications:
                with st.expander(f"Application #{app['id']} - {app['full_name']} ({app['application_date']})"):
                    col1, col2 = st.columns(2)
                    
                    with col1:
                        st.markdown("**📋 Personal Information**")
                        st.write(f"**Name:** {app['full_name']}")
                        st.write(f"**ID:** {app['id_number']}")
                    ...
                    with col2:
                        ...
                        st.write(f"**Monthly Income:** ${app['monthly_income']}")
                        st.write(f"**Occupation:** {app.get('occupation', 'N/A')}")
                        st.write(f"**Interests:** {', '.join(app.get('interests', [])) if app.get('interests') else 'N/A'}")
                    
                    st.markdown("**💭 Additional Information**")
                    if app.get('hobbies'):
                        st.write(f"**Hobbies:** {app['hobbies']}")
                    if app.get('reason'):
                        st.write(f"**Reason:** {app['reason']}")
                    
                    st.caption(f"Application date: {app['application_date']}")
        
        # Download button
        st.markdown("---")
        if st.button("📥 Download all applications (JSON)"):
            json_str = json.dumps(applications, indent=4, ensure_ascii=False)
            st.download_button(
                label="💾 Download JSON",
                data=json_str,
                file_name=f"novacard_applications_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
